...
def solve_air3d_relative(
    ...
    # disturbance_mode="min" -> adversarial Air3D (one vehicle pursues).
    # disturbance_mode="max" -> COOPERATIVE avoidance (both vehicles turn to avoid);
    #   its BRT is the pair's unavoidable-collision set even under full cooperation,
    #   which is a SOUND subset of a cooperative multi-vehicle joint BRT (audit C2).
    ...
    grid = hj.Grid.from_lattice_parameters_and_boundary_conditions(
        ...
        periodic_dims=2,  # theta is an angle on [-pi, pi] (audit H1)
    )
...
def solve_dubins3d(tau_steps, x_discretization, y_discretization, theta_discretization,
                   x_bounds, y_bounds, theta_bounds, velocity, omega_max, radius):
    from lvfm.ground_truths import Dubins3DAvoid
    times = np.concatenate(([0.0], np.array(tau_steps))) * -1.0
    dynamics = Dubins3DAvoid(velocity=velocity, omega_max=omega_max, control_mode="max",
                             disturbance_mode="min")
    grid = hj.Grid.from_lattice_parameters_and_boundary_conditions(
        hj.sets.Box(np.array([x_bounds[0], y_bounds[0], theta_bounds[0]]),
                    np.array([x_bounds[1], y_bounds[1], theta_bounds[1]])),
        shape=(x_discretization, y_discretization, theta_discretization),
        periodic_dims=2,
    )
    states = grid.states[..., :3]
    values0 = jnp.linalg.norm(states[..., :2], axis=-1) - radius
    solver_settings = hj.SolverSettings.with_accuracy(
        "very_high", hamiltonian_postprocessor=hj.solver.backwards_reachable_tube)
    return hj.solve(solver_settings, dynamics, grid, times, values0)
